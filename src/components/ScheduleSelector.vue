  ...
  <div v-else class="wrapper">
    ...
    <div class="row" style="margin-top: 10px">
      ...
      <div class="col-lg-4 select-container">
        <select v-model="currentSelect" class="form-control local-t">
    ...
    <div id="tree_stage" class="treeview" style="user-select: none">
      <div class="list-group">
        ...
        <div v-if="currentSelect === 'GroupSearch'" class="row" style="padding: 25px 15px 15px">
          <a v-for="group in groups" v-show="matchSearch(group)" :key="group" :href="`/psuti/schedule-open/list?type=group&value=${encodeURIComponent(
            group
        ...
        <div v-if="currentSelect === 'TeachersSearch'" class="row" style="padding: 25px 15px 15px">
          <a v-for="teacher in teachers" v-show="matchSearch(teacher)" :key="teacher" :href="`/psuti/schedule-open/list?type=teacher&value=${encodeURIComponent(
            teacher
...
const groups = computed(() => [
  ...new Set(allGroups.value.map((g) => g.group)),
]);
const teachers = computed(() => [
  ...new Set(allTeachers.value.map((t) => t.teacher.trim())),
]);
...
const filteredTeachers = computed(() =>
  allTeachers.value.filter(
    (t) => t.department === currentSelectDepartment.value
  )
);

const hasExtramural = computed(() =>
  allGroups.value.some(
    (g) =>
      g.subdivision === currentSelectFaculty.value && g.study_form === "Заочная"
  )
);
